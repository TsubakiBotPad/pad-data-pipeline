...
class LSTextConverter(BaseTextConverter):
    ...
    def multi_attribute_match_convert(self, ls):
        ...
        min_colors = self.attributes_to_str(attributes[:min_match])
        alt_colors = self.attributes_to_str(attributes[1:]) if len(attributes) > min_match else None
        all_colors = self.attributes_to_str(attributes)
    ...
    def passive_stats_type_atk_all_hp_convert(self, ls):
        ...
        return self.passive_stats_type_atk_all_hp_text(hp_pct, atk_mult, ls.types)
    ...
    def team_build_bonus_convert(self, ls):
        intro = self.fmt_stats_type_attr_bonus(ls)
        card = '[{}]'.format(self.concat_list_and(map(str, ls.monster_ids)))
    ...
    def color_combo_bonus_damage_convert(self, ls):
        attr_text = self.fmt_multi_attr(list(set(ls.attributes)))
        return self.color_combo_bonus_damage_text(ls.bonus_damage, ls.min_combo, attr_text)
