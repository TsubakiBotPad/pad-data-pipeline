    ...
    def __init__(self, width, height, name, buffer, encoding):
        ...
        if self.encoding.strideInBits:
            if self.encoding.strideInBits == 32:
                ...
                intermediatePackedPixels = struct.unpack(
                    "<{}B".format((self.width * self.height * self.encoding.strideInBits) // 8), self.buffer)
                self.packedPixels = []
...
class TextureWriter(object):
    ...
    penultimateChunk = struct.pack("<H32L", 0x0, 0x45747600, 0x6F537458, 0x61777466, 0x45006572, 0x726F7078, 0x20646574,
                                   ...
                                   0x75747865, 0x54206572, 0x206C6F6F, 0x77777728, 0x646F632E, 0x74617779, 0x632E7374,
                                   0x702F6D6F, 0x656A6F72, 0x2F737463, 0x7A7A7570, 0x612D656C, 0x642D646E, 0x6F676172,
                                   0x742D736E, 0x75747865, 0x742D6572, 0x296C6F6F, 0x3391286F)
    ...
    def unpackPixels(cls, texture, targetBitDepth):
        ...
        bitShifts = [sum(bitsPerChannel[channelIndex + 1:]) for channelIndex, channelBitCount in
                     enumerate(bitsPerChannel)]
        bitMasks = [(((2 ** bitCount) - 1) << bitShift) for bitCount, bitShift in zip(bitsPerChannel, bitShifts)]
        conversionTables = [cls.bitDepthConversionTable[currentBitCount][targetBitDepth] for currentBitCount in
                            bitsPerChannel]
        ...
        if texture.encoding == RAW:
            ...
            if texture.encoding.hasAlpha:
                if cls.trimmingEnabled:
                    width, height, flatPixelArray = cls.trimTransparentEdges(flatPixelArray, width, height,
                                                                             texture.encoding.channels)
                if cls.blackeningEnabled:
                    flatPixelArray = cls.blackenTransparentPixels(flatPixelArray, width, height,
                                                                  texture.encoding.channels)
            ...
            if any(flatPixelArray):
                ...
                if len(palette) <= (2 ** 8) and not texture.encoding.isGreyscale:
                    ...
                    pngWriter = png.Writer(width, height, alpha=texture.encoding.hasAlpha,
                                           greyscale=texture.encoding.isGreyscale, bitdepth=targetBitDepth,
                ...
                binaryFileData = bytes(pngFileByteArray[:-finalChunkSize]) + cls.penultimateChunk + bytes(
...
class TextureReader(object):
    ...
    def decryptAndDecompressBinaryBlob(cls, binaryBlob):
        ...
        binaryBlob = bytearray(
            byte ^ decryptionKey for byte in bytearray(binaryBlob[cls.encryptedTextureHeaderFormatSize:]))
        ...
        while (offset + cls.textureBlockHeaderSize) < len(binaryBlob):
            ...
            if magicString == cls.unencryptedTextureMagicString:
                ...
                for textureManifestIndex in range(0, numberOfTexturesInBlock):
                    ...
                    startingOffset, width, height, name = struct.unpack(cls.textureManifestFormat, binaryBlob[
                                                                                                   textureManifestStart:textureManifestEnd])
                    ...
                    try:
                        ...
                        if encodingIdentifier not in cls.encodings:
                            print("{name} is encoded with unrecognized encoding \"{encoding}\".".format(name=name,
                                                                                                        encoding=re.sub(
                                                                                                            r'^(-?0X)',
                                                                                                            lambda
                                                                                                                x: x.group(
...
def getSettingsFromCommandLine():
    ...
    outputGroup.add_argument("--outdir", metavar="OUT_DIR",
    ...
    outputGroup.add_argument("--outfile", metavar="OUT_FILE",
                             help="The output file to save to. Specifying this forces 'file decoder' mode.",
                             action=call(settings.setOutputFile))
...
def getOutputFileName(suggestedFileName):
    ...
    try:
        ...
        outputFileName = "{} ({}){}".format(outputFileWithoutExtension, getOutputFileName.filesWritten[outputFileName],
                                            outputFileExtension)
    except KeyError:
